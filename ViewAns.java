...
public class ViewAns  extends JFrame implements MouseListener ,ActionListener ,KeyListener , FocusListener,WindowListener
{
    ...
    JFrame f;    
    JPanel p,pa;
    JLabel lpic,la ,areply,tnm[],tid[],lque[],lans[],dt[],error,lid;
    JTextArea a[];
    ...
    srch=new JButton(new ImageIcon("C:\\Users\\gurvishal\\Documents\\NetBeansProjects\\KnowledgePoint\\src\\knowledgepoint\\pictures\\srch.png"));
      id=new JTextField();             id.setFont(new Font("Times New Roman",0,15));id.setBounds(1020,70,80,20);        
      lid=new JLabel("Faculty ID ");             lid.setFont(new Font("Times New Roman",0,15));lid.setBounds(950,70,80,20);         
      lid.setForeground(new Color(0,0,153));
      error=new JLabel();
    ...
    back1.setBounds(50,150,30,30); back1.setMargin(new Insets(0,0,0,0));
    back1.setCursor(new Cursor(HAND_CURSOR));
    logout.setCursor(new Cursor(HAND_CURSOR));
    back.setCursor(new Cursor(HAND_CURSOR));
        p.setLayout(null);
    f.setSize(1369,729);
    lpic.setBounds(0,0,1369,4920);

    srch.setBounds(1100,50,105,44);      srch.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));  srch.setBorder(BorderFactory.createEmptyBorder());
    back.setBounds(10,65,89,28);               back.setBorder(BorderFactory.createEmptyBorder());
...
f.add(sp, BorderLayout.CENTER);
    ...
    Object o=e.getSource();
        ...
        c=DriverManager.getConnection("jdbc:derby://localhost:1527/StudentInformation","simran","12345");
                ...
                rs=st.executeQuery("select * from quesans where tid='"+id.getText().toUpperCase()+"'");
                String name=null;
                rs.last();
                int i=rs.getRow();
                   t=new JTextField[i];
                ...
                for(i=0;i<t.length;i++)
                {
                    ...
                    dt[i].setFont(new Font("Times New Roman",0,15));
lque[i].setBounds (10,y,65,30);    t[i].setBounds(75,y,1100,30);     tid[i].setBounds(1185,(y-15),200,30);      tnm[i].setBounds(1185,(y),150,30); dt[i].setBounds(1185,(y+15),200,30);
lans[i].setBounds(10,(z+40),60,30);   a[i].setBounds(75,(z+40),1200,90);       a[i].setBorder(BorderFactory.createLineBorder(Color.black));
                    ...
                    dt[i].setForeground(new Color(153,0,153));
                    y=z+140;
                    z=y+10;
                ...
                rs.afterLast();
                String ques=null,ans=null,nm=null,uu=null,sdt=null;
                int ij=0;
                while(rs.previous())
                {
                ques=rs.getString(1);
            ...
            {
                System.out.println("error catch in err= "+ex.getMessage());
            }
                if(t.length==0)
                {
